...
def generate_bibfile(query: str = "", reference: str | None = None) -> str:
    """
    ...
    reference : `str`, optional
        Name of file to read bib entries from that will be guaranteed
        to be retained even if they are not found in query results.
    ...
    bibdata = create_bibentries(res)
    print(f"Got {len(res['hits'])} records max:{MAXREC} produced {len(bibdata.entries)} bibentries.")

    # Look for missing entries and replace.
    if reference:
        with open(reference) as fd:
            refbib = BibliographyData.from_string(fd.read(), "bibtex")

        ref_keys = set(refbib.entries.keys())
        found_keys = set(bibdata.entries.keys())
        missing = ref_keys.difference(found_keys)
        if missing:
            print("Restoring entries that were missed by query: ", ", ".join(missing))
            for missed in missing:
                bibdata.add_entry(missed, refbib.entries[missed])

            # Re-sort the entries.
            bibdata = BibliographyData(entries={k: bibdata.entries[k] for k in sorted(bibdata.entries)})
...
if __name__ == "__main__":
    ...
    result = generate_bibfile(args.query, args.reference)
