...
class ProcSpie(AuthorTextGenerator):
    ...
    def generate(self) -> str:
        ...
        authors = []
        for author in self.authors:
            # Offset counter by 1 to index into chars.
            labels = [chars[affil_to_number[affil] - 1] for affil in author.affiliations]
            authors.append(rf"\author[{','.join(labels)}]{{{author.full_latex_name}}}")

        affiliations = []
        for affil, number in affil_to_number.items():
            affiliations.append(rf"\affil[{chars[number - 1]}]{{{affil}}}")
...
class ADASS(AuthorTextGenerator):
    ...
    def generate(self) -> str:
        ...
        for author in self.authors:
            aindexes.append(f"%\\aindex{{{author.family_name},~{author.initials.replace(" ", "~")}}}")
