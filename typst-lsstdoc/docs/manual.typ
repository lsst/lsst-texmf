...
#figure(
  table(
    ...
    [`source-version`], [`auto`], [Repository version of the source, shown after the change record; `auto` uses the build's `--input source-version=...` (derived from git by the technote Makefile) and shows it only in drafts, since released documents rely on the change record. An explicit value always renders.],
